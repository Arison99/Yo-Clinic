...
const [selectedDate, setSelectedDate] = useState(new Date());
 ...
 const fetchdata = async () => {
    try {
      const response = await axios.get(`http://localhost:27017/single/${id}`, {
        headers: {
...
const handlesave=async()=>{
  ...
  try {
    ...
    const response = await axios.patch(`http://localhost:27017/update-date`, {_id:id,date:selectedDate}, {
      headers: {
        'Content-Type': 'application/json',
        authorization: localStorage.getItem('jwt'),
      },
  ...
  const handleSubmit = async(e) => {
    ...
    try {
        const response = await axios.patch(`http://localhost:27017/update-medicine`, {_id:id,medicine,about}, {
          headers: {
            'Content-Type': 'application/json',
            authorization: localStorage.getItem('jwt'),
          },
